...
> 🧠 Exam Focus: `az deployment group create` targets a resource group. For subscription scope use `az deployment sub create`.
...
### 12.9 Pricing Deep Dive (Illustrative – Always Verify Current Portal Pricing)
...
> 🧠 Exam Focus: Distinguish event (state change broadcast — Event Grid) vs message (command/intent — Service Bus) vs stream (continuous high-volume telemetry — Event Hubs).